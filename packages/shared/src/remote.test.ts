...
describe("remote", () => {
  ...
  it("derives backend urls from hosted app pairing links", () => {
    expect(
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.bibcode.codes/pair?host=https%3A%2F%2Fdesktop.tailnet.ts.net%3A44342%2F#token=pairing-token",
      }),
    ).toEqual({
      credential: "pairing-token",
  ...
  it("accepts single-letter bare hosts with ports for direct and hosted inputs", () => {
    ...
    expect(
      resolveRemotePairingTarget({
        pairingUrl: "https://app.bibcode.codes/pair?host=C%3A4100%2Fhealth#token=hosted-code",
      }),
    ).toEqual({
  ...
  it("rejects Windows and POSIX local path forms as direct backend hosts", () => {
    const localPaths = [
      ...
      "C:folder",
      "C:relative\\bibcode",
      "\\\\server\\share\\bibcode",
      "\\\\?\\C:\\Users\\dev\\bibcode",
      "\\\\.\\pipe\\bibcode",
      "\\Users\\dev\\bibcode",
      "/var/run/bibcode.sock",
      "./relative/bibcode",
      "../relative/bibcode",
      ".\\relative\\bibcode",
      "..\\relative\\bibcode",
  ...
  it("rejects local paths carried by hosted pairing requests with their source", () => {
    ...
    try {
      resolveRemotePairingTarget({
        pairingUrl: "https://app.bibcode.codes/pair?host=C%3A%5CUsers%5Cdev%5Cbibcode#token=code",
      });
    } catch (cause) {
      error = cause;
    }
  ...
  it("rejects unsupported hosted pairing backend protocols", () => {
    ...
    try {
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.bibcode.codes/pair?host=ftp%3A%2F%2Fremote.example.com#token=pairing-token",
      });
    } catch (cause) {
      hostError = cause;
    }
