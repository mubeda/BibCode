...
describe("RPC wire fixture exporter", () => {
  it("writes a deterministic, complete fixture tree", async () => {
    ...
    expect(manifest.methods).toHaveLength(121);
    ...
    expect(manifest.typedFailureFixtures).toHaveLength(278);
    expect(manifest.staleMethodIdentifiers).toEqual([
      "projects.add",
      "projects.list",
      "projects.remove",
    ]);
    expect(manifest.fixtures).toHaveLength(370);
    expect(manifest.fixtures).toEqual([...manifest.fixtures].toSorted());
    expect(Object.keys(manifest.schemaFingerprints)).toHaveLength(348);
