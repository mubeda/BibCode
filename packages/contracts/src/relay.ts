...
}).annotate({ description: "Links an authenticated cloud user to a BiBCode environment." });
...
>()("@bibcode/contracts/relay/RelayClientPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in BiBCode Connect user.",
  ),
);
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "BiBCode Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking BiBCode environments and connecting authorized clients.",
  );
