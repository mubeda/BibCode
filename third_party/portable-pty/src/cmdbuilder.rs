...
#[cfg(windows)]
impl CommandBuilder {
    /// Assign the spawned process to these Windows Job Objects as part of
    /// `CreateProcessW`.
    ///
    /// The caller must keep every handle valid until `spawn_command` returns.
    pub fn job_list(&mut self, jobs: &[std::os::windows::io::RawHandle]) {
        self.job_list = jobs.iter().map(|job| *job as isize).collect();
    }

    pub(crate) fn get_job_list(&self) -> &[isize] {
        &self.job_list
    }
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn job_list_preserves_handle_values_for_process_creation() {
        let mut cmd = CommandBuilder::new("dummy");
