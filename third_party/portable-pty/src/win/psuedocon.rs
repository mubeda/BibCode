...
impl PsuedoCon {
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // BiBCode streams terminal bytes without emulating a terminal
    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        ...
        let jobs = cmd.get_job_list();
        let attribute_count = if jobs.is_empty() { 1 } else { 2 };
        let mut attrs = ProcThreadAttributeList::with_capacity(attribute_count)?;
        attrs.set_pty(self.con)?;
        if !jobs.is_empty() {
            attrs.set_job_list(jobs)?;
        }
