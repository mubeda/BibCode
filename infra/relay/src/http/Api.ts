...
import { encodeOAuthScope } from "@bibcode/shared/oauthScope";
...
import { normalizeRelayIssuer } from "@bibcode/shared/relayJwt";
  ...
  Effect.gen(function* () {
    ...
    return {
      relayDpop: Effect.fn("relay.auth.dpop_client")(function* (httpEffect, { credential }) {
        ...
        yield* Effect.annotateCurrentSpan("relay.auth.mode", "dpop");
        return yield* httpEffect.pipe(
          Effect.provideService(RelayClientPrincipal, {
            userId: verified.sub,
            token,
