...
`normalize_worktree_path_key(path, platform)` normalizes separators and trailing separators, lowercases Windows drive/UNC comparison keys, and preserves POSIX case. Present paths are canonicalized before key generation. Missing paths use the normalized Git path. `WorktreeRepositoryKey` and `WorktreeKey` are lowercase SHA-256 hex over a version prefix, normalized common-directory key, a NUL separator, and normalized path key.

- [ ] **Step 1: Add failing parser and identity tests**

Cover NUL and legacy records with spaces, Git C-style quoted paths, embedded newline escapes, detached/unborn, bare, locked with and without reason, prunable with and without reason, duplicate fields, missing record terminator, over-limit output, Windows drive/UNC equivalence, POSIX case distinction, and deterministic opaque keys.
...
- [ ] **Step 4: Connect BiBCode-created Git mutations to catalog invalidation**

Give `GitVcsRpcServices` an optional catalog mutation observer. After successful `vcs.createWorktree`, call `note_managed_creation` then invalidate. After successful legacy `vcs.removeWorktree`, increment the mutation epoch and invalidate. Never turn a failed observational refresh into a failed mutation response.
...
### Task 6: Implement idempotent adoption and branch reconciliation
...
- Produces: `worktree.adopt` returning exactly one active ordinary workspace thread and catalog-driven durable branch updates.
...
- rejects a second canonical thread for the exact normalized server path;
...
- Produces: `createWorktreeEnvironmentAtoms`, pure discovery derivation, availability lookup by scoped thread, add-one/add-all commands, and focus refresh support.
...
Use `createEnvironmentSubscriptionAtomFamily` for snapshots and `createEnvironmentRpcCommand` for refresh, policy, and adoption. Key commands serially by `environmentId + projectId`; key refresh as single-flight. Export the new `./state/worktrees` package subpath.
...
Prove guard-before-callback ordering, duplicate transition coalescing, degraded no-op, recovery clearing only for the same path/repository, removing precedence, and bounded reaper behavior.
...
Add `WorkspaceUnavailableError` to the affected contract RPC error unions. Inject the same registry into orchestration, terminal, Git VCS, and `WorkspaceRpcDependencies`. Internal catalog/removal calls use `GitRepository` directly and bypass client-path guards.
...
**Interfaces:**
- Consumes: Thread ID, fresh catalog generation, opaque plan token, explicit removal mode/confirmations, runtime quiescer, and verified Git primitives.
- Produces: `worktree.getRemovalPlan`, `worktree.removeFromBibCode`, `worktree.remove`, retry-safe detach/delete transactions, and partial cleanup outcomes.
...
The plan token is SHA-256 over a version prefix, physical project, thread, repository key, worktree key or missing-path identity, generation, availability, dirty counts, lock state, and sorted prune-impact keys. Removal always reruns preflight and requires the digest to match.
...
`getRemovalPlan` resolves the current server state and never mutates. `removeFromBibCode` acquires the project lock and removing guard, requests bounded quiesce, dispatches `WorktreeDetachResolved` regardless of quiesce result, clears client-path guard state for the deleted thread, invalidates the catalog, and returns `gitOutcome: "not-requested"`.
...
For present destructive mode, perform verified Git mutation before durable detach and release `removing` back to the refreshed state on failure. For missing cleanup, attempt targeted cleanup, conditionally prune only with a matching confirmation, then detach even on cleanup failure. Preserve the checked-out branch.

- [ ] **Step 6: Advertise the capability only after the complete server surface exists**

Set `worktreeCatalog: true` in the environment descriptor and control-stream config. Add descriptor tests proving older payloads decode false and the production server advertises true. Register read/operate scopes and update `ACTIVE_RPC_METHODS` for all three methods.
...
Read availability from client-runtime by scoped thread. Pass a single `workspaceUnavailable` value through the active chat surface instead of adding independent catalog subscriptions to every component. Render one banner and use it to disable workspace-dependent controls.
...
rg -n "subscribeWorktreeCatalog|vcs\.refreshWorktreeCatalog|worktree\.updateDiscoveryPolicy|worktree\.adopt|worktree\.getRemovalPlan|worktree\.removeFromBibCode|worktree\.remove" packages/contracts/src/rpc.ts apps/server/src/rpc/methods.rs apps/server/src/auth/scope.rs apps/server/src/production/worktree_catalog_rpc.rs
```

Expected: no unresolved markers, no client path authority, no recursive worktree deletion fallback, no browser-owned Git removal, and every method appears in contracts, registry, scope map, and handler registration.
...
Report every validation command and result, the final commit range, any skipped platform-specific coverage, and residual risks. Confirm all eleven acceptance criteria in the approved design against an automated test or an explicitly identified manual verification.
