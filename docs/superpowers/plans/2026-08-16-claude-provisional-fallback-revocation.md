...
Task 1 also makes the neighboring `targeted_activity_rpc_writes_only_the_selected_claude_stop_task_subtree` fixture's former assumption explicit: its child was provisional, so adding `agent-unmapped` correctly revoked it. Preserve that test's exact selected-subtree purpose by adding one authenticated nested `PostToolUse` hook after the existing child `PreToolUse` and `SubagentStart`, with `agent_id: "agent-a"`, `tool_use_id: "tool-agent-child"`, and `tool_response: {"status":"async_launched","agentId":"agent-child"}`. The existing availability wait then proves the promoted exact child, and every later selected-cancellation/provider-byte assertion remains unchanged. Do not weaken Task 1's revocation policy or make the observer wait for a target that must remain provisional.
