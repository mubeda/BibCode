...
- **Modify:** `apps/server/src/git/manager/operations.rs` — the stage-partial and discard-partial executors.
...
- **Modify:** `apps/server/src/production/git_manager_rpc.rs` — replace the `gitManager.stagePartial` / `gitManager.discardPartial` stub handlers with real ones.
- **Modify:** `apps/server/tests/production_git_manager_rpc.rs` — integration coverage for staging a partial selection and for partial discard.
...
- [ ] **Step 11.8: Add the discard-partial executor.**
	...
	(The reference implementation builds an already-reversed patch and applies it without `--reverse`; `--reverse` is the same operation with the intent visible. Record whichever you use in the phase notes.) Partial discard is destructive, so the handler must fail closed on a stale generation rather than discarding the wrong lines. Test both the success path and the stale-generation rejection.

- [ ] **Step 11.9: Implement the handlers.** In `apps/server/src/production/git_manager_rpc.rs` replace the `gitManager.stagePartial` and `gitManager.discardPartial` stubs, delegating to `stage_partial` / `discard_partial`. Both are mutations and use the operate scope, both validate the pathspec with the existing `validate_pathspecs` helper before doing anything, and both log stable codes plus lengths and counts only — never a file path, a patch body, a line of file content or git stderr.

- [ ] **Step 11.10: Add the integration tests.** In `apps/server/tests/production_git_manager_rpc.rs`, against a temporary repository: stage a two-line selection out of a four-line change and assert `git diff --cached` contains exactly the selected lines; discard a one-line selection and assert the file content and that the other change survives.
...
- [ ] **Step 11.12: Stack-specific verification.** In a scratch repository, stage a partial selection through the RPC and verify with `git diff --cached` and `git diff` that exactly the intended lines moved; repeat for an untracked file and for a file without a trailing newline. Repeat against a remote-hosted project (spec § 10 requires both).
...
- **Method contract for PHASE-14:** the two methods are `gitManager.stagePartial` and `gitManager.discardPartial` (both gated on `gitManagerPartialStaging`); there is no separate unstage-partial method.
- **Selection index contract for PHASE-14:** `selectedLines` is a set of **0-based indices over the unified-diff body of the file's current working-tree diff** — the same coordinate space the client's parsed patch uses. The client sends indices, never line contents. PHASE-14's gutter must derive its indices from the same diff payload the server produced, or the selection will not line up.
