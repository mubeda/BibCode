...
- [ ] **Step 12.2: Author the first failing test.**
	...
	Also pin the index-stability rule that the rest of this phase depends on: `resolveStashIndex(entries, sha)` returns the entry's **current** `index`, or `null` when the sha is gone. Stash indices shift on every push and drop, so an index is only ever valid against the list it came from — the UI selects by sha and resolves the index at dispatch time.
...
- [ ] **Step 12.8: Add the stash-diff pane test, then implement it.**
	...
	Render the selected stash's changed-file list from the `GitManagerStashEntry` already returned by `gitManager.getStashes` — do **not** issue a call for it. For the selected path, read the patch through `gitManagerEnvironment.getDiff({ environmentId, input: { cwd, source: { _tag: "stash", index, path } } })`, resolving `index` from the current list with `resolveStashIndex(entries, selectedStashSha)` at request time. Hand the patch to `getRenderablePatch(patch, "git-manager-stash")` from `apps/web/src/lib/diffRendering.ts` and render through the existing `AnnotatableCodeView` path used by `apps/web/src/components/DiffPanel.tsx`. Do NOT add a second diff renderer or a second worker pool — `DiffWorkerPoolProvider` already bounds the pool.

	Assert: the `raw` fallback branch of `RenderablePatch` renders its `reason` rather than throwing; `resolveStashIndex` returning `null` renders an "entry no longer present" state and refetches the list instead of issuing a `getDiff` with a stale index; and a **stale-index `GitManagerOperationError` from the server is an expected outcome** — PHASE-09 maps index → `stash@{index}` → sha and fails structurally when the index no longer resolves, so the UI refetches `getStashes` and re-resolves rather than surfacing a hard failure.
...
- [ ] Only PHASE-00's declared methods are used: `rg -n "getStashDiff|listStashes|applyStash|popStash|dropStash" apps/web/src` returns nothing. The stash file list comes from `gitManager.getStashes`; a per-path stash patch comes from `gitManager.getDiff` with `{ _tag: "stash", index, path }`.
- [ ] Stash selection is stored and passed as a **sha**; the index is resolved from the current list at dispatch time, and a stale-index error refetches instead of failing hard. `rg -n "selectedStashIndex" apps/web/src` returns nothing.
...
- **Contract hazard — `GitManagerDiffSource`'s stash arm identifies a stash by `index`, which is not stable.** A stash index shifts on every push and drop, so a cached or persisted index can silently resolve to a different entry. PHASE-09 mitigates it server-side by mapping index → `stash@{index}` → sha and failing with a structured error when the index no longer resolves. This phase mitigates it client-side by selecting on **sha**, resolving the index from the current `getStashes` payload at dispatch time via `resolveStashIndex(entries, sha)`, and treating a stale-index error as an expected outcome that refetches the list — never as a hard failure. Downstream phases issuing a stash `getDiff` must do the same.
  **Recommendation for the coordinator:** identifying the stash arm by `sha` instead of `index` would remove this whole class of race, since a sha is stable and the server already resolves one. That is a `packages/contracts/src/gitManager.ts` change and therefore a PHASE-00 decision — recorded here rather than acted on.
