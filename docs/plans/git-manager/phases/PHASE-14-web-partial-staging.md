...
5. `Skill(skill="vercel-react-best-practices")` — *drag-select fires per pointer move over a virtualized diff*
6. `Skill(skill="web-design-guidelines")` — *the gutter must be keyboard-operable, not pointer-only*
