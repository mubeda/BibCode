...
- Settings shows **Remote Servers** with **Connect to a host** and **Share this
  ...
  bracketed IPv6 and stable address/port IDs; if Tailscale is installed in its
  packaged location, discovery must not depend on shell `PATH`. The local-machine
