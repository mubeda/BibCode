...
Only the exact unsupported-control response revokes targeted support for the
current runtime generation; generic failure, timeout, and connection closure do
not. Revocation clears all exact handles before later clicks are admitted and
never falls back to the root interrupt path.
