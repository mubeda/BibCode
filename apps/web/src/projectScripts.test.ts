...
import {
  ...
  setupProjectScript,
} from "@bibcode/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("builds default runtime env for scripts", () => {
    ...
    expect(env).toMatchObject({
      BIBCODE_PROJECT_ROOT: "/repo",
      BIBCODE_WORKTREE_PATH: "/repo/worktree-a",
    });
  });
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        BIBCODE_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.BIBCODE_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.BIBCODE_WORKTREE_PATH).toBeUndefined();
  });
