...
impl Harness {
    ...
    async fn shutdown(self) -> Result<(), bibcode_server::ServerError> {
...
async fn project_create_resolves_home_relative_paths_and_reuses_canonical_duplicates() {
    ...
    let outcome = AssertUnwindSafe(async {
        ...
        let named_user_path = "~bibcode-final-review-user/project";
        rpc_request(
            &mut socket,
            "14",
            "orchestration.dispatchCommand",
...
async fn expect_failure_with_deadline(
    ...
    match message {
        ServerMessage::Exit {
            ...
            match &cause[0] {
                bibcode_server::CauseItem::Fail { error } => error.clone(),
...
async fn expect_interrupt_exit(
    ...
    match message {
        ServerMessage::Exit {
            ...
            assert_eq!(
                cause,
                vec![bibcode_server::CauseItem::Interrupt { fiber_id: None }]
            );
        }
